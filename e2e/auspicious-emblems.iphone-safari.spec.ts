...
async function expectAuspiciousEmblemsHealthy(page: Page) {
  ...
  for (let index = 0; index < 4; index += 1) {
    ...
    expect(visibleHeight).toBeGreaterThanOrEqual(40);
  }
...
test.describe("iPhone Safari auspicious emblem scatter", () => {
  test("renders real, visible, non-blocking emblems across core routes", async ({ page }) => {
    await makeAppOfflineSafe(page);

    for (const route of CORE_ROUTES) {
      await page.goto(route, { waitUntil: "domcontentloaded" });
      expect(await page.evaluate(() => window.innerWidth)).toBe(390);
      await expectAuspiciousEmblemsHealthy(page);
    }
  });
});
