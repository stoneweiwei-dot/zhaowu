...
const report = await readFile(new URL("../docs/change-reports/ZW-WEB-2026.08.31-r13.md", import.meta.url), "utf8");
const agents = await readFile(new URL("../AGENTS.md", import.meta.url), "utf8");

test("public footer always exposes current release and cumulative update count", () => {
  assert.match(stats, /ZW-WEB-2026\.08\.31-r13/);
  assert.match(stats, /updateNumber:\s*13/);
...
test("every production backend change requires a matching change report", () => {
  assert.match(report, /# 昭梧更新報告｜ZW-WEB-2026\.08\.31-r13/);
