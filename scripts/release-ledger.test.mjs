...
const report = await readFile(new URL("../docs/change-reports/ZW-WEB-2026.09.06-r65.md", import.meta.url), "utf8");
const agents = await readFile(new URL("../AGENTS.md", import.meta.url), "utf8");
test("public footer always exposes current release and cumulative update count", () => {
  assert.match(stats, /ZW-WEB-2026\.09\.06-r65/);
  assert.match(stats, /updateNumber:\s*65/);
...
test("every production backend change requires a matching change report", () => {
  assert.match(report, /# 昭梧更新報告｜ZW-WEB-2026\.09\.06-r65/);
