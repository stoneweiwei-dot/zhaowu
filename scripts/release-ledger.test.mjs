...
const report = await readFile(new URL("../docs/change-reports/ZW-WEB-2026.09.15-r144.md", import.meta.url), "utf8");
...
test("public footer always exposes current release and cumulative update count", () => {
  ...
  assert.match(stats, /updateNumber:\s*144/);
...
test("every production frontend change requires a matching change report", () => {
  assert.match(report, /# 昭梧更新報告｜ZW-WEB-2026.09.15-r144/);
