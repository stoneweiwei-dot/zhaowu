...
test("full reports render answer first, then supporting detail and body attention, while decree image delivery stays server-side", async () => {
  ...
  assert.match(resultView, /reportSections \? <FocusedReportSections sections=\{reportSections\} result=\{result\} \/>/);
...
test("free decree text remains available when image generation fails", async () => {
  ...
  assert.ok(imagePosition > decreePosition, "free decree text must render before the optional image");
  assert.doesNotMatch(resultView.slice(imagePosition), /\{decreeCouplet\}/);
...
test("site shell keeps the approved Zhaowu lotus brand mark without random page-wide ornament scatter", async () => {
