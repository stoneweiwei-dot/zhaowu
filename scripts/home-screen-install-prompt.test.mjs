...
test("manifest and Apple web-app metadata support a clean home-screen launch", () => {
  ...
  assert.ok(manifest.icons.some((icon) => icon.sizes === "180x180" && icon.src === "/apple-touch-icon-r20.png"));
  assert.ok(manifest.icons.some((icon) => icon.sizes === "192x192" && icon.src === "/icons/zhaowu-lotus-192.png"));
  assert.ok(manifest.icons.some((icon) => icon.sizes === "512x512" && icon.src === "/icons/zhaowu-lotus-512.png"));
  ...
  assert.match(html, /href="\/icons\/zhaowu-lotus-192\.png"/);
  assert.match(html, /href="\/icons\/zhaowu-lotus-512\.png"/);
  assert.doesNotMatch(html, /apple-touch-icon\.png\?v=/);
  assert.doesNotMatch(html, /manifest\.webmanifest\?v=/);
