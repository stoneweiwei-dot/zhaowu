...
test("manifest and Apple metadata support home-screen launch", () => { assert.equal(manifest.id, "/"); assert.equal(manifest.display, "standalone"); assert.equal(manifest.start_url, "/"); assert.equal(manifest.short_name, "昭梧"); assert.ok(manifest.icons.some((icon) => icon.sizes === "180x180")); assert.ok(manifest.icons.some((icon) => icon.sizes === "192x192")); assert.ok(manifest.icons.some((icon) => icon.sizes === "512x512")); assert.match(html, /rel="manifest" href="\/manifest\.webmanifest"/); assert.match(html, /apple-mobile-web-app-capable/); assert.match(html, /rel="apple-touch-icon" href="\/apple-touch-icon-r34\.png"/); });
