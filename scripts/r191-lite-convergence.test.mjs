...
test("r191 keeps one canonical official product while adopting Lite mobile discipline", async () => {
  ...
  assert.match(result, /buildDecisionReportModel/);
  assert.match(registry, /正式站 × 昭梧 Lite/);
  assert.match(registry, /zhaowu-guide\.ston1004\.chatgpt\.site/);
});

test("r191 does not loosen Storage write freeze as a side effect of the visual closeout", async () => {
