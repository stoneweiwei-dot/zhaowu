...
- 工作去留題必須直接說明「能判／不能可靠判」與成立條件；只有「工作要穩」「看承載」這類原則句不得通過。
- 感情「還有沒有發展空間／能不能繼續／會不會復合」必須直接回答可判範圍，並落到可驗證的聯繫、投入、承諾／下一步；泛用「多溝通」不得通過。
- 財務「最該防什麼／風險在哪」必須點名具體風險類型或明確說明資料不足；「財運有起伏、保守一點」不得通過。
- 二選一若缺少同一組可比資料，必須明說暫不強選並列出缺失條件；不得用命盤替現實資料補造 A/B 結論。
- 複合問題必須同時滿足所有已辨識要求；例如「不知道時辰＋適合什麼工作」不能只回答時辰限制或只回答職業方向。
- 第一段與支援段若出現「一切都是最好的安排」「相信自己」「順其自然」「宇宙會給你」等泛用雞湯，內容驗收直接失敗。
