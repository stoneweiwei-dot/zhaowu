...
## 2026-09-25 r202 五行對應 × 真正補五行 supersession

- ACTIVE：首頁 Today 的五行穿衣不再只停在顏色提示；選中的五行至少同屏說明其核心功能／運動方向，並顯示五色、五音、五氣的對應。
- ACTIVE：完整 `/daily-colors` 頁擴展五季、五方、五味、五臟腑、五志、五體、五藏、五勞，定位為傳統文化／生活映射與學習層。
- ACTIVE：「補五行」固定先判功能狀態：有利但不足／有利但受阻／已足夠／過旺需制衡／表面缺失但不宜增加／先要母氣或通關；禁止「缺什麼補什麼」。
- ACTIVE：五色、五音、五氣、方位、飲食、物件等不得反向決定格局、喜用、病藥或歲運；個人命盤仍沿用 CURRENT 子平主鏈與既有 `ZW-FIVE-ELEMENT-FUNCTION-1.0` 功能訓練。
- 保護邊界：本輪只擴展現有五行穿衣／知識呈現，不改四柱、格局、喜用、Auth、payment、Supabase schema／Storage 或報告持久化契約。
