...
站主指定連結預覽要對上這張雙蓮金殿圖。舊 `og.jpg` 是 1008×566 山水，而且頁面沒有 `og:image`，分享時平台抓不到正確卡片。r59 只改預覽卡與 meta，不改產品流程。
...
回滾本 r59 commit 即可恢復舊山水 `og.jpg` 與無 Open Graph meta 的 `index.html`。不需要資料庫回滾。
