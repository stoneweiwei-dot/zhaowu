...
### 背景音樂改走 jsDelivr，HEAD 不再 401
- GitHub raw 對音檔回 `Content-Disposition: attachment` 與 `sandbox` CSP，iPhone 進度條停在 00:00。
- 串流與清單 URL 改指向 `cdn.jsdelivr.net/gh/...`，inline `audio/mpeg`。
- `/api/owner-music?stream=1` 的 HEAD 與 GET 一樣 307，Safari 探 metadata 不再拿 401。
...
同時正式站把背景音樂 307 到 GitHub raw，iPhone 拿到 attachment／sandbox 後進度停在 00:00；HEAD stream 還回 401。改 jsDelivr + HEAD=GET。

## 影響範圍

- `src/components/d60-reliability-gate.tsx`
- `src/lib/specialist-reading.ts` 本卷總覽文案
- `api/owner-music.js` 播放 URL
