...
- 問事分析完成後，青玉小龍主動提示可先看「依據／風險／下一步」，但不強制彈開助手面板。

## 為什麼改

避免定位失敗時顯示虛假城市、降低出生城市選取摩擦，把生辰保存後的長頁壓縮成先讀核心、需要時再展開完整細節，並讓問事完成後有清楚的下一步導覽。
...
- 對應 location／city／home/report／assistant 回歸測試與 public release metadata

## 受保護範圍

不修改四柱、真太陽時、節氣、十神、起運等 deterministic calculation truth；不修改回答引擎、Auth、Payment、Supabase schema、夜模式或其他首頁功能。青玉小龍只增加分析完成後的本地導覽提示，不新增外部模型或付費 API。

## 驗證狀態

- Source contract：已補 IP 回退文案、CityHit Gate／focus、#bazi 預設收合與小龍分析後接續提示測試。
- Deploy gate：待 PR CI。
