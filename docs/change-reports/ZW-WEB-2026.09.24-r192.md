...
- 登入影片單次最多播放 15 秒，然後停在封面；當次登入流程不重播。
- 聲音控制改為 44px 喇叭圖示，保留無障礙名稱；後台短片時長上限由 5 秒調為 15 秒。
- 登入影片清單僅顯示 MP4／WebM，圖片與封面不混入。
- 正式站吸收 Lite 參考頁的 560px 手機閱讀寬度與克制紙色，保留原單一核心流程。
...
- 站主 cookie、登入 API、既有媒體原件及 Storage 寫入凍結。

大檔來源（200–500 MB）的續傳、轉碼與發佈尚未實作；Storage 寫入凍結期間後台上傳仍停用。
...
- canonical Production `/`、`/login`、`/updates`、`/quiz/cultivation-destiny` smoke；核對登入影片最長 15 秒和喇叭控制。
