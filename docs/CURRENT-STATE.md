...
- r156：站主最新指令將 r149 的獨立右下角播放器 UI `SUPERSEDED`。播放邏輯與完整五鍵歌單控制保留，但 UI 只可存在於青玉小龍助手內；全站右下角只保留一個可拖動、吸邊、位置持久化的龍助手。助手間歇隨機顯示導覽氣泡或帶播放／下一首的迷你音樂氣泡，展開後才顯示完整播放器與站內導覽。禁止重新掛回第二個 fixed music dock。
