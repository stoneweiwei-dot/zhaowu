...
- Gallery/背景资产管理能力保留；全站应用页读取 Supabase 启用背景，每天只选择一张进入实际画面，固定壁纸与有效排程优先，失败时回退仓库内建宋画。
...
- 全站应用页与登入页使用站主启用背景做每日轮播；固定壁纸优先，其次为有效排程，再到一般轮播。`/wallpaper-song.jpg` 只作读取失败或无启用资产时的内建回退。
...
- 动态 owner 背景重新参与前台 shell，但每次只下载最终选中的一张；背景不会反向影响内容、命理判断或命诰匹配。
...
## 2026-09-01 — STO-13 B 版背景、懒载入与 QA 隔离

- 客户前台每天确定性选择一张启用背景；固定壁纸优先，有效排程次之，一般轮播最后，不使用 `background-attachment: fixed`。
