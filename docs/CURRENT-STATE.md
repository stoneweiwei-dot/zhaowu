...
## 2026-09-20 r170 客戶回答收口

- 首答、追問與完整報告共用 `customerAnswer`，歷史資料唯讀相容；不重算四柱。
- 天賦／職業方向只在已成立複合結構與清楚承接條件下給傳統推論，附內部依據；未知時辰、時間邊界或證據不足時不補造能力。
- 出生資料完整不等於解讀可靠；舊「判斷把握較高」及按題型自動堆疊行動／風險套話 SUPERSEDED。
- 同一題的新時間追問取代舊年份，保留原主題。手機正文單欄，技術資料預設收起；開完整報告後首答不再重複顯示。
- 身體欄保留，但沒有症狀／檢查資料時只標示無法判定，不自動羅列疾病與部位；舊固定健康象義長文只供歷史相容。
- 本次修改回答組合及呈現；不改排盤核心、認證、支付、資料庫 schema、圖片原件。
