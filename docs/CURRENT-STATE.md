...
视觉母版是暖米宣纸／宋式图谱体系。首页按「一幅完整东方长卷」处理，而不是一张又一张不透明卡叠在白底上：
...
- 非首页应用路由继续使用稳定的封闭暖宣纸底，避免后台与工具页可读性下降。
...
- 首頁以同一張宋式山水作可見的縱向背景，宣紙卡改為較薄、較透、較緊凑的單一閱讀流。
- 客戶不再看到真太陽時與子時換日選項。後台固定先做真太陽時校正，再以午夜換日：23:00–23:59 同日，00:00–00:59 次日。
...
- 頁面只回答兩件事：平時怎樣做事、壓力來時怎樣反應；「軌道 A／B、融合星評、正向寄語」及底部融合長文已移除。
...
- 首頁在主分析表單上方顯示一條緊凑的「性格兩面」入口，繁中／簡中／英文都可直接進入 `/tianji-dual`。
- 入口不是大型產品卡，不恢復舊工具牆，也不改首頁唯一主分析表單、排盤核心或報告流程。

## 2026-08-29 — iPhone 首頁按核准畫面收口

- 最新明確指令修正：首頁必須保留「性格兩面」緊凑入口；改的是原「雙軌命盤」名稱與舊版設計，不是移除入口。
- 入口使用暖米宣紙、朱砂細邊與「平時怎樣做事，壓力來時怎樣反應」白話說明，繁中／簡中／英文都可進入 `/tianji-dual`。
...
## 2026-08-29 — 報告雜誌與契約對齊

- 完整報告 `FocusedReportSections` 接收 `result`，可渲染雙語修心建議漫畫。
- `scripts/ui-contract.test.mjs` 已對齊現況 `result` prop，不得用舊契約阻斷 CI。
- 不改排盤核心、auth、payment、Supabase schema。
- 生產 alias 必須指向含此改動的當前 `main` HEAD；不得停在舊 production SHA。
