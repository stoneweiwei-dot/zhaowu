...
- r157：站主最新 UI 指令把 r149 的獨立右下角播放器 UI `SUPERSEDED`。播放邏輯、完整五鍵控制、owner playlist、loop／shuffle 偏好與 Safari 手勢解鎖全部保留，但只存在於青玉小龍助手內。小龍是唯一 floating entry：預設右下角、可拖動、左右吸邊、位置本機持久化；未展開時間歇隨機顯示導覽或迷你音樂 speech bubble。禁止重新掛回第二個 fixed music dock。全部 26 首站主曲目已保留原檔後統一降低整體響度、限制峰值並收斂高頻；播放器預設音量由 24% 降至 16%，未來新上傳曲目沿用同一柔化流程。
