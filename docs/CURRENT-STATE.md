...
- 全站应用页与登入页恢复固定 `/wallpaper-song.jpg` 宋画山水背景；图鉴海报不再参与页面背景。
...
- 3s 雙生蓮 loading 使用 r26 恢復的原始舊宣紙並蒂蓮影片與同源首幀備援；最遲三秒放行。v13 分片只保留舊建置相容，不再作前台播放來源。
...
- 首页「前世今生」从错误的 `/tianji-dual` 改为真正的 `/yizhangjing`；旧 `/tianji-dual` 仅保留运行兼容，不再对客户作为独立「性格两面」产品或入口。
...
## 2026-09-01 — 分组说明 + 五行功能趣味测验

- 首页三门与趣味测验卡片统一增加“你会知道 / 最擅长看”说明，繁中、简中、英文分别写自然、简洁的客户语言。
- 「性格两面」不再是独立客户分组；旧 `/tianji-dual` 仅作为兼容路径存在，不得出现在首页、青玉小龙或产品文案。
- 趣味测验新增「五行功能测验」：12 题只看最近 1–3 个月的当前状态，输出一个主轴与可选辅轴；Q9 用于识别同一功能的过度版本，结果不展示分数。
- 五行功能测验不等同八字喜用神，不使用“缺什么补什么”，不调用图片生成；只输出行动建议、观察标准、过度提醒与象征性灵光色彩提示。
- 五行功能测验结果自动保存到当前装置的「我的纪录」，不新增 Supabase schema。

## 2026-09-01 — STO-13 管理与 QA 隔离（背景项已被 r25 撤回）

- r25 已依站主要求撤回客户前台每日轮播。后台背景能力仍保留；没有新指令不得重新启用图鉴海报作为首页或登入页背景。
- 站主后台背景区首次只读取最近一张；点「查看上传历史」后才按每页最多 12 张载入。
- 多图上传保留一次选择能力，并逐张显示各自进度与失败，不以一张失败中断其余图片。
- Owner／客户正式报告列表在查询层与客户端同时排除 QA、test、e2e 记录；正式客户资料、统计口径与详情懒载入不变。
- 客户卡片默认只露出查看与刷新；复制、命诰图与删除集中在主动展开的管理操作，不增加手机端常驻按钮。

## 2026-09-02 — r25 视觉还原

- 恢复 r23 的固定宋画山水页面背景，只撤回 r24 的 shell/CSS 背景接线。
- 保留 r23 注册回调修复、r24 后台多图上传/历史分页、QA 隔离与报告卡精简。
- r26 已由站主确认恢复「梧桐树下双生并蒂莲开花、天降四华」原片；以新媒体路径替换金色内嵌登入按钮的视频，静态备援使用原片首帧。

## 2026-09-02 — r26 原始并蒂莲动画

- 原片采用旧宣纸色、梧桐树、同茎双莲从含苞到盛开、天花交错飘落。
- 使用 twin-lotus-restored-r26.mp4 与同名 jpg，540×960；完整五秒原片加速至约 2.73 秒，保留三秒硬退出。
- 去掉为遮挡旧影片内嵌按钮而设置的黑色遮罩；没有重绘或添加花朵。
- 原始文件 SHA-256：824f03165c38e95d0613ca1f33321155e41181aa404a7da1aac76be2904c58d9。
