...
唯一批准母版：**暖米宣纸・宋式山水融合 UI（2026-08-27 owner-approved 9:16 mockup）**。名称“昭梧・四柱绘意报告风”继续保留为产品视觉名称，但实际前台以这套母版为最终视觉来源。

使用方式：

- 背景与前景必须是同一个“连续宣纸世界”：后台壁纸真实可见，卡片只用同色系半透明宣纸／绢层，禁止明显白卡、奶油色差平台或重毛玻璃重新盖住背景；
- 主色：暖米白、旧宣纸黄、淡茶灰、墨褐；交互强调只用朱砂红与青玉绿；**紫黑、深紫命盘卡、乌金黑大底不再是当前前台主视觉**；
- Header 使用温暖半透明宣纸；语言选中态使用青玉绿；主 CTA 使用朱砂红；正文使用墨色；
- 首页 hero、分析表单、结果卡、命诰图库、完整报告、双轨工具统一使用同一纸色、低阴影、细金茶色边线，不能看起来像不同系统拼接；
- 完整报告与双轨页旧紫色 CSS 只保留为历史依赖，active path 必须由 `src/approved-parchment-ui-v3.css` 最后覆盖，禁止恢复紫黑主题；
- 天龙八部／佛教吉祥法器继续可作低层级附属装饰，但不再使用压暗黑金主视觉；装饰需低饱和、低透明，不遮字、不切断背景连续性；
- 完整报告每个实际出现的内容区块继续配置一只水彩青玉小龙；三组共 27 个表情按内容语义稳定选择，小龙只作情绪提示，加载失败不得阻塞或遮挡正文；
- 命诰图区域本身也必须是暖宣纸容器，图库缩略图与生成图不再放在紫色平台中；真实个人命诰图的生成算法与命理逻辑不因 UI 换色而改变；
...
- 全站随机吉祥符号仍只作低层级装饰，不得遮挡文字、输入或按钮；
- iPhone 390–430 px 优先，严禁横向溢出、卡片断层、色差平台和可读性下降；
- 真实个人命诰图仍按命局全局推导，不能“看到某一柱就套某一个物件”。

详细生成图规则仍见 `docs/PAID-REPORT-STYLE-v1.0.md`；前台视觉覆盖的最终锁文件是 `src/approved-parchment-ui-v3.css`。

## 6. Issue / 旧任务规则

旧迁移与重复任务不得自动复活。只有当前 production 仍能复现同一 FAIL 才允许重新处理。

长期交接与尚未完成的新功能可继续保留；任何旧 PR 如果明确要求恢复固定九页、旧幽灵 SVG / 旧品牌小 logo（不含当前批准的现代吉祥符号散布）、Netlify / AppDeploy production，则应关闭而不是合并。
