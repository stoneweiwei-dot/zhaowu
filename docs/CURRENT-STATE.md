...
- 后台背景库可设为壁纸；Supabase 站主背景优先于 weekly fallback，但**首页目前不渲染任何图库／weekly 壁纸**。
...
- **首页采用整页封闭宣纸成品版**：从 Header 到 Footer 都处于同一暖米宣纸页面，不渲染后台图库壁纸、weekly fallback 或其他全屏背景图；页面区块之间即使有间距，也只能露出同一宣纸底，禁止露出另一张背景图；
- **首页取消全页随机吉祥小图散布**，并移除工具卡、完整报告中的松散小 logo／贴纸式装饰，避免独立小图卡在文字或卡片边缘；必要品牌识别、功能性「问小龙」可保留；
- 首页所有文字承载面必须为高密度暖宣纸／米白实体卡，禁止为“看背景”而降低到玻璃透明度；正文优先级高于装饰；
- 主色：暖米白、旧宣纸黄、淡茶灰、墨褐；交互强调只用朱砂红与青玉绿；**紫黑、深紫命盘卡、乌金黑大底不再是当前前台主视觉**；
- Header 使用近实体暖宣纸；语言选中态使用青玉绿；主 CTA 使用朱砂红；正文使用墨色；
- 首页 hero、分析表单、结果卡、命诰图库、完整报告、双轨工具统一使用同一纸色、低阴影、细金茶色边线，不能看起来像不同系统拼接；
- 首页最终视觉锁为 `src/home-sheet-ui-v5.css`，必须最后导入并覆盖旧 wallpaper / glass / purple active path；`src/approved-parchment-ui-v3.css` 与 `src/visual-readability-lock-v4.css` 作为基础层保留；
- 首页以外页面暂时仍可使用站主背景系统，除非站主后续明确要求全站一并关闭；不得把“首页无背景图”误改成删除后台图库能力；
- 天龙八部／佛教吉祥法器等装饰不再放在首页页面层上漂浮；如其他独立体验仍使用，必须低饱和且不遮字；
...
详细生成图规则仍见 `docs/PAID-REPORT-STYLE-v1.0.md`；首页视觉覆盖的最终锁文件是 `src/home-sheet-ui-v5.css`。

## 6. Issue / 旧任务规则

旧迁移与重复任务不得自动复活。只有当前 production 仍能复现同一 FAIL 才允许重新处理。

长期交接与尚未完成的新功能可继续保留；任何旧 PR 如果明确要求恢复固定九页、旧幽灵 SVG / 旧品牌小 logo、首页随机装饰散布、Netlify / AppDeploy production，则不应合并其冲突部分。
