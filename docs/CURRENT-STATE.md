...
- Loading 前台 100% 使用站主提供的並蒂蓮池原片（FCA79DBB…mp4，5.04s 720×1280）转成 `/intro/loading-owner-r40.mp4`（H.264 yuv420p muted CRF26 787KB，建置时由 8 个 base64 分片还原），首帧海报 `/intro/loading-owner-r40.jpg`；`LOTUS_BLOOM_MS=5000`，最迟 5300ms hard-exit。v13 分片只保留旧建置相容，不再作前台播放來源。
