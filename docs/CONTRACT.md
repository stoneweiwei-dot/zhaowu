...
`usefulProvisional=true` 時，粗候選只可作內部占位與「待覆核」說明；**不得派生顏色、方位、時段、寵物、擺設、職業吉凶等生活取象結論**。這些欄位在結果頁與完整報告必須留白或明示「正式取用尚未完成」。
...
有 `XAI_API_KEY` 時的 Grok 擴寫也必須保持同一個**固定九段順序**，不得因模型擴寫變成 8 段或合併段落。第 7 段受 `usefulProvisional` 約束：正式取用未完成時，只寫待覆核，不列具體顏色、方位、時段或寵物。
