...
两边不能直接对话。**唯一源码真相是本仓库；当前产品状态以 [docs/CURRENT-STATE.md](./docs/CURRENT-STATE.md) + 当前 `main` + 当前 Production 为准。**

## 所有 APP / AI / Agent 的统一入口

无论由 ChatGPT、Codex、Grok、AppDeploy、GitHub-connected agent、部署 Agent 或未来任何平台接手昭梧，第一优先级都必须先读取并遵守根目录 [AGENTS.md](./AGENTS.md)，随后读取 [docs/INSTRUCTION-REGISTRY.md](./docs/INSTRUCTION-REGISTRY.md)。
...
1. [docs/INSTRUCTION-REGISTRY.md](./docs/INSTRUCTION-REGISTRY.md) — 当前有效／历史／曾等待权限但已失效的指令状态
2. [docs/CURRENT-STATE.md](./docs/CURRENT-STATE.md) — 当前生产架构、已完成项、未完成项、优先级
3. [docs/STONE-R6.2.1-CURRENT-MASTER.md](./docs/STONE-R6.2.1-CURRENT-MASTER.md) — 当前命理母指令
4. [docs/CONTRACT.md](./docs/CONTRACT.md) — 技术接口历史与现行契约；若与 current main / tests / CURRENT-STATE 冲突，以后者为准
...
- Loading、Logo、Hero、登录方式等已收口功能默认冻结，除非站主给出更新的明确指令。
- 客户完整报告内容契约只认 `summary / body` 与 `FOCUSED-REPORT.md`；不得恢复旧固定 01–09 session。程序化 tabs / swipe cards 可作为阅读层，但不能让模型重新生成多套互相冲突的报告。
- 纯视觉修改不得阻塞排盘、登录、报告读取等核心故障。
- 旧“等待权限”的补丁不能自动当待办；先按 Instruction Registry 检查它是否已经实现、被取代或属于旧架构。
- 每次修改都要说明：**当前可复现问题 → 改动 → 验收结果 → production commit/deployment（如涉及 runtime）**。
...
| GPT | 产品、文案、报告与视觉规范、命理治理／规则入库、审查，以及工具权限允许的代码／文档收口 |
| Stone | 产品拍板与最终实机验收 |

这不是权限墙；如果 Stone 明确要求某一方直接处理网站，应以当前工具能力处理，但不得绕过 `AGENTS.md`、Instruction Registry、当前锁文件与 CURRENT-STATE。
...
禁止搬回 AppDeploy 的 MutationObserver / form-title 双标题系统，也禁止复制旧 AppDeploy `src/account.js` auth hotfix 到当前 React/Supabase 架构。
