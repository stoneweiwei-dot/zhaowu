...
async function loadGitRuntime() {
  ...
  if (!gitRuntimePromise) {
    gitRuntimePromise = import("isomorphic-git").then((mod) => mod.default || mod);
  }
...
export async function readOwnerMusicManifest() {
  // Public reads do not need a Git smart-HTTP handshake. Reading the branch raw
  // manifest directly keeps the hot GET path off the legacy smart-HTTP URL stack on Node 24.
...
export async function renameOwnerMusicTrack(secret, trackId, nextName) {
  ...
  return withRepo(async (dir, git) => {
    ...
    writeManifestFile(dir, next);
    await git.add({ fs, dir, filepath: MANIFEST_PATH });
    const commitSha = await commitAndPush(dir, privateKey, `owner-music: rename ${trackId}`);
    next.commitSha = commitSha;
    return next;
  });
}
...
export async function deleteOwnerMusicTracks(secret, trackIds) {
  ...
  return withRepo(async (dir, git) => {
    ...
    writeManifestFile(dir, next);
    await git.add({ fs, dir, filepath: MANIFEST_PATH });
    const commitSha = await commitAndPush(dir, privateKey, `owner-music: delete ${ids.length} tracks`);
    next.commitSha = commitSha;
    return next;
  });
}
...
async function removeScratch(dir, uploadId, git) {
...
export async function saveOwnerMusicChunk(secret, file) {
  ...
  return withRepo(async (dir, git) => {
    ...
    await git.add({ fs, dir, filepath: `${TRACK_DIR}/${filename}` });
    await git.add({ fs, dir, filepath: MANIFEST_PATH });
    await removeScratch(dir, uploadId, git);
    const commitSha = await commitAndPush(dir, privateKey, `owner-music: add ${filename}`);
    track.url = publicTrackUrl(filename, commitSha);
    next.commitSha = commitSha;
    return next;
