...
export function sanitizeChartInput(raw) {
  ...
  if (!/^(19|20)\d{2}-(0[1-9]|1[0-2])-(0[1-9]|[12]\d|3[01])$/.test(birthDate)) {
    return { ok: false, error: { code: "INVALID_INPUT", message: "birthDate must use YYYY-MM-DD from 1900-2099.", details: { field: "birthDate" } } };
  }
  ...
  if (birthCalendar === "solar" && !validGregorianDate(birthDate)) {
    return { ok: false, error: { code: "INVALID_INPUT", message: "Invalid Gregorian birthDate.", details: { field: "birthDate" } } };
  }
