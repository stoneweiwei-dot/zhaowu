...
export const zhaowuInstructionDatabase: InstructionRule[] = [
  ...
  {
    id: 'ZW-AUX-PALACES-BOUNDARY-1.0',
    title: '胎元／命宮／身宮算法與越權防護協議',
    status: 'production',
    layer: 'boundary',
    priority: 6,
    purpose: '保留胎元、命宮、身宮作為傳統輔助象義，同時阻止不同流派算法混用或以輔助宮位反向改寫子平主判。',
    rules: [
      '胎元、命宮、身宮每次使用都必須記錄來源、流派、曆法、起算方式與時辰邊界；不同算法不得合併後當成同一個確定性結果。',
      '即使算法結果確定，胎元、命宮、身宮仍只屬輔助象義層；月令、根氣、透藏、格局、調候、病藥、流通、承載與歲運主判優先。',
      '來源若只提供口訣或 App 結果而未說明算法，先標 reference／candidate；不得假裝已驗證為唯一正統排法。',
      '胎元受刑沖穿害、命宮見華蓋空亡、身宮落某干支等，只能作補充觀察，必須有主盤結構與現實證據交叉支持後才可提高權重。',
      '輔助宮位與主盤衝突時，保留差異並降低輔助層權重，不得用輔助宮位推翻四柱、日主、月令、格局或用神。'
    ],
    guards: [
      '禁止單憑胎元斷體弱、多病、災厄或壽命。',
      '禁止單憑命宮華蓋、空亡等斷財運差、淡泊名利、婚姻差或人生層級。',
      '禁止單憑身宮判一生財富、社會地位、健康與壽命。',
      '禁止把某個 App 的身宮／命宮算法直接升格為全站唯一算法。'
    ],
    outputContract: [
      '若啟用胎元／命宮／身宮，必須同時顯示：算法來源、結果、輔助象義、主盤交叉證據、衝突點與「不得單獨定吉凶」邊界。'
    ]
  },
  {
    id: 'ZW-NATAL-LUCK-PHASE-1.0',
    title: '原局定體／歲運定勢與成破救應協議',
    status: 'production',
    layer: 'bazi',
    priority: 9,
    purpose: '把原局結構與大運流年分層，允許歲運改變力量比例、成破救應與顯化強度，但禁止用歲運倒寫出生格局。',
    rules: [
      '原局先定月令、根氣、透藏、格局、調候、病藥、流通與承載；大運、流年、流月只在此底盤上改變階段力量、可用資源、成破救應與事件觸發。',
      '歲運可以令原局某一結構暫時得救、受破、轉清、轉濁、加重或形成新通路，但不得因此把出生四柱與原局基本格局機械改名。',
      '若採某流派所稱「動態格局／歲運格局變化」的語言，只能描述階段性運作狀態，必須明標流派，並與原局格局分欄呈現。',
      '用神／藥神在歲運中的優先級可以因病勢、承載與通路改變；但必須指出它此時是扶用、疏通、救應、制化、爭用、助病、受制還是引動新病。',
      '原局有結構而歲運未啟動時，只能說潛在；歲運啟動但原局沒有相應承載或做功路徑時，不得硬造事件。'
    ],
    guards: [
      '禁止「走到某五行大運＝原局格局自動變成另一格」。',
      '禁止「喜用五行出現＝一定吉／必發」。',
      '禁止只看大運流年名稱，不回查原局病藥、根氣、制化、流通與承載。'
    ],
    outputContract: [
      '歲運分析固定分欄：原局底盤 → 階段力量變化 → 成破救應 → 具體觸發 → 承載／新病 → 現實驗證點。'
    ]
  },
  {
    id: 'ZW-OWNER-MATERIAL-APP-20260905-1.0',
    title: '2026-09-05 命理 App 截圖來源校正守則',
    status: 'production',
    layer: 'boundary',
    priority: 8,
    purpose: '把本批壬辰、胎元命宮身宮與格局歲運截圖中的可用結構吸收進系統，同時隔離單柱宿命論、醫療財婚恐嚇與未註明流派算法。',
    rules: [
      '本批現代 App 截圖屬 OWNER_MATERIAL／二手來源，不是古籍原典，也不是已校準個案；只能提供候選規則與待核主張。',
      '壬辰的確定性部分只保留干支事實：辰藏戊、乙、癸；對壬日主而言分別為七殺、傷官、劫財。其餘人格、婚財、健康與生肖配對句不得由日柱單獨輸出。',
      '壬辰、壬寅時等古訣與壬騎龍背類特殊標籤必須回到完整四柱、月令、透干根氣、成破救應、調候病藥與歲運驗證；只有形態不得宣稱純格成立。',
      '來源中的「用神平衡五行」只保留為低階概念，不取代昭梧既有病藥功能法；正式判定仍須先辨病，再分扶抑、調候、通關、制化等功能並檢查真用條件。',
      '來源中的時辰吉凶、生肖婚配、疾病部位、財富與婚姻斷語只作原文研究材料；未通過獨立校驗不得進自動報告。'
    ],
    guards: [
      '禁止把本批 App 文案當作權威經典。',
      '禁止單一壬辰日柱固定人格、富貴、婚姻、疾病、壽命或職業。',
      '禁止把不同頁面、不同時辰、不同流派的條文拼接成同一人的完整結論。'
    ],
    outputContract: [
      '凡引用本批來源，只能輸出已校正事實、條件化結構或明確標示的來源原文；不得讓原文越過主判與證據鏈。'
    ]
  },
  {
