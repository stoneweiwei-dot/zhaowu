const INTERNAL_SENTENCE =
  /全站回答契約|全站回答契约|本頁只使用|本页只使用|排序依據|排序依据|月份名稱只是|月份名称只是|命理月以節氣|命理月以节气|月份排序是已接入|月份排序是已接入|通用句|資料未接入|资料未接入|尚未完成|待覆核|待复核|不是完整子平|不是喜用神|方法透明|報告編號|报告编号|隱藏算法|隐藏算法|STONE Core|已排定的柱|旺衰底盤|旺衰底盘|流通粗候選|流通粗候选|調候粗候選|调候粗候选|不偽造午時|不伪造午时/;
