...
export const SITE_RELEASE_FALLBACK = {
  ...
  latestSummary: "收官批次：首頁高風險區塊 fail-open；完整報告移除 prompt／dashboard 式文案；命理證據治理對齊 R6.2.2，核心排盤不改。",
  details: {
    "zh-Hant": [
      "首頁三個高風險區塊改為獨立錯誤邊界；任一區塊異常時只降級該區，不再觸發整頁 Router Error。",
      "已保存生辰維持 fail-open／lazy；報告只保留自然成品語言，R6.2.2 收緊證據與執行邊界；Supabase Storage 清理成果與寫入凍結維持不變。",
    ],
    en: [
      ...
      "Saved-birth analysis stays fail-open and lazy; report labels are now product copy rather than prompt-like UI, while R6.2.2 tightens evidence governance without rewriting chart calculation.",
    ],
  },
} as const;
