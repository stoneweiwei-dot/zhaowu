...
export const SITE_GUIDE_ROUTES = ["/", "/#analysisForm", "/tianji-dual", "/account", "/login"] as const;
...
const COPY: Record<Locale, GuideCopy> = {
  "zh-Hant": {
    analysis: ["要問工作、感情、時機或人生選擇，從首頁的分析表單開始。", "開始分析"],
    dual: ["想看天機命宮與達摩一掌經的雙軌結果，請進入「天機・雙軌命盤」。", "進入雙軌命盤"],
    account: ["已保存的出生資料與最近報告都在「我的昭梧」。", "查看我的昭梧"],
    login: ["登入後才能保存報告、沿用命盤續問或進入站主後台。", "前往登入"],
    home: ["我可以帶你去分析、雙軌命盤、已保存報告或登入頁。", "返回首頁"],
  },
  "zh-Hans": {
    analysis: ["要问工作、感情、时机或人生选择，从首页的分析表单开始。", "开始分析"],
    dual: ["想看天机命宫与达摩一掌经的双轨结果，请进入“天机・双轨命盘”。", "进入双轨命盘"],
    account: ["已保存的出生资料与最近报告都在“我的昭梧”。", "查看我的昭梧"],
    login: ["登录后才能保存报告、沿用命盘继续提问或进入站主后台。", "前往登录"],
    home: ["我可以带你去分析、双轨命盘、已保存报告或登录页。", "返回首页"],
  },
  en: {
    analysis: ["For work, relationships, timing or a life decision, start with the analysis form on the home page.", "Start analysis"],
    dual: ["For the Tianji Life Palace and Dharma Palm results together, open the Dual Destiny Chart.", "Open dual chart"],
    account: ["Your saved birth details and recent reports are in My Zhaowu.", "Open My Zhaowu"],
    login: ["Sign in to save reports, continue with the same chart, or access the owner area.", "Sign in"],
    home: ["I can guide you to an analysis, the dual chart, saved reports, or sign-in.", "Go home"],
  },
};
...
const DUAL = /雙軌|双轨|天機|天机|一掌經|一掌经|命宮|命宫|六道|前世|dual|tianji|palm|life palace/i;
const ACCOUNT = /我的昭梧|保存|報告|报告|紀錄|记录|歷史|历史|帳戶|账户|account|saved|report|history/i;
...
function isAllowedRoute(value: unknown): value is SiteGuideRoute {
  return typeof value === "string" && (SITE_GUIDE_ROUTES as readonly string[]).includes(value);
}

export async function askSiteGuide(message: string, locale: Locale, pathname: string): Promise<SiteGuideAnswer> {
  ...
  return {
    reply: typeof body.reply === "string" && body.reply.trim() ? body.reply.trim().slice(0, 500) : defaultSiteGuide(locale).reply,
    route: isAllowedRoute(body.route) ? body.route : null,
    cta: typeof body.cta === "string" && body.cta.trim() ? body.cta.trim().slice(0, 80) : null,
