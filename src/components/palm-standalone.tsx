...
const COPY = {
  "zh-Hant": {
    ...
    formTitle: "排出你的四宮足跡", formLead: "填入最少必要資料，直接排盤。結果只在本機即時計算，不登入、不保存。",
  ...
  "zh-Hans": {
    ...
    formTitle: "排出你的四宫足迹", formLead: "填入最少必要资料，直接排盘。结果只在本机即时计算，不登录、不保存。",
  ...
  en: {
    ...
    formTitle: "Map your four-palace trail", formLead: "Enter only the essentials and calculate immediately. Everything runs on this device and is not stored.",
...
export function PalmStandalone() {
  ...
  return (
    <main className="mx-auto max-w-4xl space-y-6 pb-10 sm:space-y-8">
      <section className="relative overflow-hidden rounded-[2rem] border border-[#b99655]/40 bg-[#17140f] shadow-[0_28px_90px_rgba(24,18,10,.28)]">
        <div className="absolute inset-0 bg-[radial-gradient(circle_at_82%_18%,rgba(39,103,93,.34),transparent_34%),radial-gradient(circle_at_10%_90%,rgba(143,48,39,.18),transparent_38%),linear-gradient(135deg,#18140f_0%,#232019_48%,#112b28_100%)]" />
        ...
        <div className="relative grid gap-7 px-5 pb-6 pt-7 sm:px-8 sm:pb-8 sm:pt-9 lg:grid-cols-[1.35fr_.65fr] lg:items-center lg:gap-10 lg:px-10 lg:py-10">
          <div>
          ...
          <div className="relative mx-auto hidden aspect-square w-full max-w-[15rem] place-items-center lg:grid" aria-hidden>
        ...
        <div className="relative border-t border-[#d8b66d]/15 bg-black/10 px-4 py-3 sm:px-8">
      ...
      <section className="rounded-[1.75rem] border border-[#b99755]/28 bg-[#fffaf0]/92 p-4 shadow-[0_18px_52px_rgba(70,49,26,.09)] sm:p-6">
        ...
        <form onSubmit={onSubmit} className="relative overflow-hidden rounded-[1.4rem] border border-line/80 bg-cream/92 p-5 sm:p-7" noValidate>
          ...
          <div className="relative">
            <div className="grid gap-5 sm:grid-cols-2">
              <label className="block text-sm font-medium text-ink">
                {copy.name}
                <input value={name} onChange={(event) => setName(event.target.value.slice(0, 40))} placeholder={copy.namePh} className="mt-2 min-h-12 w-full rounded-xl border border-line bg-white/72 px-4 text-base outline-none transition focus:border-cinnabar focus:ring-2 focus:ring-cinnabar/10" />
              </label>
              <label className="block text-sm font-medium text-ink">
                {copy.date}
                <input type="date" min="1900-01-31" max={maxDate} value={date} onChange={(event) => setDate(event.target.value)} className="mt-2 min-h-12 w-full rounded-xl border border-line bg-white/72 px-4 text-base outline-none transition focus:border-cinnabar focus:ring-2 focus:ring-cinnabar/10" />
              </label>
            </div>
            ...
            <fieldset className="mt-5">
              ...
              <div className="mt-3 grid gap-3 sm:grid-cols-2">
                {(["male", "female"] as const).map((value) => (
                  <label key={value} className={`flex min-h-14 items-center gap-3 rounded-xl border px-4 py-3 text-sm transition ${gender === value ? "border-cinnabar bg-cinnabar/7 text-ink shadow-[inset_3px_0_0_#8f3027]" : "border-line bg-white/60 text-ink-soft"}`}>
            ...
            <label className="mt-5 block text-sm font-medium text-ink">
              {copy.hour}
              <select value={hour} onChange={(event) => setHour(event.target.value)} className="mt-2 min-h-12 w-full rounded-xl border border-line bg-white/72 px-4 text-base outline-none transition focus:border-cinnabar focus:ring-2 focus:ring-cinnabar/10">
            ...
            <button type="submit" className="mt-6 min-h-12 w-full rounded-full bg-[#7f2f28] px-6 text-sm font-semibold tracking-[0.12em] text-[#fff7e7] shadow-[0_14px_30px_rgba(111,36,30,.2)] transition hover:bg-[#68251f] focus:outline-none focus:ring-2 focus:ring-cinnabar/35 focus:ring-offset-2">
              {copy.submit}
            </button>
      ...
      {result ? (
        <section ref={resultRef} className="scroll-mt-20 space-y-5 overflow-hidden rounded-[1.9rem] border border-[#b99755]/35 bg-cream/95 shadow-[0_28px_80px_rgba(62,43,24,.14)]" aria-live="polite">
          <div className="relative overflow-hidden border-b border-[#b99755]/20 bg-[#1b1812] px-5 py-6 sm:px-8 sm:py-7">
