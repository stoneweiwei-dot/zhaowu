...
function useVisitorContext() {
  ...
  const loadWeather = async (latitude: number, longitude: number, timezone: string): Promise<VisitorContext> => {
    const weatherResponse = await fetch(`https://api.open-meteo.com/v1/forecast?latitude=${latitude}&longitude=${longitude}&current=temperature_2m,weather_code&timezone=auto&forecast_days=1`, { cache: "no-store" });
  ...
  const requestLocation = async () => {
    ...
    try {
      ...
      const value = await loadWeather(position.coords.latitude, position.coords.longitude, timezone);
      setVisitor(value);
    } catch {
      setVisitor(null);
