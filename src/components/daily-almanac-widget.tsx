    ...
    <section id="daily-almanac" className="zhaowu-today-guide" aria-label={labels.title}>
      <details className={`zhaowu-daily-details${embedded ? " is-embedded-open" : ""}`} open={embedded || undefined}>
        ...
        <div className="zhaowu-today-guide__expanded">
          ...
          <nav className="zhaowu-today-guide__tabs" aria-label={locale === "en" ? "Today sections" : locale === "zh-Hans" ? "今日分区" : "今日分區"}>{pageTitles.map((title, index) => <button key={title} type="button" aria-pressed={page === index} onClick={() => setPage(index)}>{title}</button>)}</nav>
