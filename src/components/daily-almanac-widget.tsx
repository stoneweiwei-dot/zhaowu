...
export function DailyAlmanacWidget() {
  ...
  const labels = locale === "en" ? { title: "Today Guide", sub: "Local time · weather · almanac rhythm", almanac: "Daily Almanac", wardrobe: "Daily Dress · Five Elements", spirit: "Daily Spirit Slip", location: "Location & weather", sacred: "Sacred day", pillars: "Stems & branches", core: "Core dynamic", yi: "Good for", ji: "Avoid", relation: "Combine · clash · penalty", good: "Supportive hours", caution: "Caution hours", colors: "Colours", jewellery: "Jewellery", mask: "Persona mask" } : locale === "zh-Hans" ? { title: "今日指引", sub: "当地时间・即时天气・今日气机", almanac: "每日黄历", wardrobe: "每日穿衣｜五行色彩", spirit: "今日灵签｜签文指引", location: "所在地｜天气", sacred: "今日圣日", pillars: "今日干支", core: "核心气机", yi: "宜", ji: "忌", relation: "合冲刑害", good: "吉时", caution: "慎时", colors: "适宜颜色", jewellery: "适宜首饰", mask: "性格面具" } : { title: "今日指引", sub: "當地時間・即時天氣・今日氣機", almanac: "每日黃曆", wardrobe: "每日穿衣｜五行色彩", spirit: "今日靈籤｜籤文指引", location: "所在地｜天氣", sacred: "今日聖日", pillars: "今日干支", core: "核心氣機", yi: "宜", ji: "忌", relation: "合沖刑害", good: "吉時", caution: "慎時", colors: "適宜顏色", jewellery: "適宜首飾", mask: "性格面具" };
  ...
  const locationName = visitor?.city || (locale === "en" ? "Local" : "本地"); const weather = `${weatherLabel(visitor?.weatherCode ?? null, locale)}${visitor?.temperature != null ? ` ${Math.round(visitor.temperature)}°C` : ""}`; const pageTitles = [labels.almanac, labels.wardrobe, labels.spirit];
  ...
  return <>
    <section id="daily-almanac" className="zhaowu-today-guide" aria-label={labels.title}>
      <header className="zhaowu-today-guide__hero"><div><p>{labels.title}</p><span>{labels.sub}</span></div><div className="zhaowu-today-guide__pager"><b>{page + 1}/3</b><button type="button" onClick={() => setPage((page + 1) % 3)} aria-label={locale === "en" ? "Next page" : "下一頁"}>→</button></div></header>
      ...
      {page === 0 ? <div className="zhaowu-today-guide__grid">
        ...
        <article className="zhaowu-today-card is-weather"><small>{labels.location}</small><strong>{locationName} · {weather}</strong><span>{seasonLabel(visitor?.latitude ?? null, now.getMonth() + 1, locale)}</span></article>
      ...
      <details className="zhaowu-daily-details"><summary>legacy contract</summary><span>當下年月日時干支</span></details>
