...
import { PaidReportCounter } from "@/components/paid-report-counter";
...
import { canPreviewStagedPaidReport } from "@/lib/report/report-tier-contract";
import { createEngineReportRecord, patchReportRecord } from "@/lib/supabase-rest";
...
const RESULT_COPY = {
  ...
  en: {
    ...
    saved: "The free result has been saved to this record.",
    saveFailed: "Saving failed.",
    saving: "Saving…",
    updateSaved: "Update saved result",
    unknown: "Unknown",
...
export function ResultView({ result }: { result: AnalysisResult }) {
  ...
  const canPreviewPaid = canPreviewStagedPaidReport(profile?.is_owner);
  ...
  async function onSave() {
    ...
    setMsg(null);
    try {
      const row = await createEngineReportRecord({ session, profile, result });
  ...
  return (
    <section id="result" className="space-y-5">
      ...
      <PaidReportCounter canPreview={canPreviewPaid} isBusy={busy !== null} onPreview={() => void onFull()} />
      {canPreviewPaid && reportSections ? (
        <FocusedReportSections sections={reportSections} />
      ) : null}
