...
function indexFor(locale: Locale) {
  return locale === "en" ? 2 : locale === "zh-Hans" ? 1 : 0;
}

export function BaziKnowledgeNotesSection() {
  ...
  return (
    <div id="mingli-knowledge" className="scroll-mt-20 space-y-5">
      <section className="seal-border rounded-2xl bg-cream p-5 sm:p-8">
        <p className="text-xs tracking-[0.26em] text-cinnabar">{tr(locale,"昭梧 · 命理小知識","昭梧 · 命理小知识","ZHAOWU · BAZI KNOWLEDGE")}</p>
        <h2 className="mt-2 font-display text-3xl leading-tight text-ink">{tr(locale,"命理知識就是知識，不混進觀世錄文章。","命理知识就是知识，不混进观世录文章。","Teaching notes stay teaching notes, separate from essays.")}</h2>
        <p className="mt-4 max-w-3xl text-[15px] leading-7 text-ink-soft">
          {tr(locale,"這裡集中放八字用途、基礎概念、天干本象、判讀步驟、神煞規則與來源辨識。它們是教學卡與方法說明，不當作站主文章，也不與觀世錄混排。","这里集中放八字用途、基础概念、天干本象、判读步骤、神煞规则与来源辨识。它们是教学卡与方法说明，不当作站主文章，也不与观世录混排。","This section contains BaZi applications, core concepts, stem imagery, reading steps, auxiliary-star rules and source checks. These are teaching cards and method notes, not editorial articles.")}
        </p>
      </section>
      ...
      <section className="seal-border rounded-2xl bg-paper p-5 sm:p-8">
        ...
        <h3 className="mt-2 font-display text-2xl text-ink">{tr(locale,"八字學用來幹嘛？","八字学用来干嘛？","What can BaZi actually be used for?")}</h3>
      ...
      <section className="seal-border rounded-2xl bg-paper p-5 sm:p-8">
        ...
        <h3 className="mt-2 font-display text-2xl text-ink">{tr(locale,"八字實際會碰到的內容","八字实际会碰到的内容","What the method actually examines")}</h3>
        <div className="mt-5 grid gap-3 sm:grid-cols-2">
      ...
      <section className="seal-border rounded-2xl bg-paper p-5 sm:p-8">
        <h3 className="font-display text-2xl text-ink">{tr(locale,"十天干本象","十天干本象","Ten Heavenly Stems")}</h3>
        <p className="mt-3 rounded-xl border border-line bg-cream px-4 py-3 text-sm leading-6 text-ink-soft">
          {tr(locale,"此處為五行／天干本象的白話轉譯，用於理解性質，不可脫離月令、旺衰、格局、病藥、透藏與歲運而單獨斷命。","此处为五行／天干本象的白话转译，用于理解性质，不可脱离月令、旺衰、格局、病药、透藏与岁运而单独断命。","These are teaching metaphors, not standalone chart judgements. Season, strength, structure, remedy, visible/hidden stems and timing still come first.")}
        </p>
        <div className="mt-5 grid gap-3 sm:grid-cols-2">
          {STEMS.map((stem) => (
            <article key={stem.key} className="rounded-xl border border-line bg-cream p-4">
              <div className="flex items-center gap-4">
                <div className={`flex h-14 w-14 shrink-0 items-center justify-center rounded-full border font-display text-3xl ${ELEMENT_CLASS[stem.element]}`}>{stem.key}</div>
                <div>
                  <h4 className="font-display text-lg text-ink">{stem.key} · {stem.image[i]}</h4>
                  <p className="mt-1 text-xs leading-5 text-ink-mute">{stem.words[i].join(" · ")}</p>
                </div>
              </div>
            </article>
          ))}
      ...
      <section className="seal-border rounded-2xl bg-paper p-5 sm:p-8">
        <h3 className="font-display text-2xl text-ink">{tr(locale,"一張知識卡怎麼讀","一张知识卡怎么读","How to read a knowledge card")}</h3>
        <div className="mt-5 grid gap-3 sm:grid-cols-5">
          {steps.map(([title,text],n) => (
            <article key={title} className="rounded-xl border border-line bg-cream p-4">
              <span className="text-xs text-cinnabar">{String(n+1).padStart(2,"0")}</span>
              <h4 className="mt-2 font-display text-lg text-ink">{title}</h4>
      ...
      <section className="seal-border rounded-2xl bg-paper p-5 sm:p-8">
        <h3 className="font-display text-2xl text-ink">{tr(locale,"神煞到底怎麼用","神煞到底怎么用","How auxiliary stars are actually used")}</h3>
        <div className="mt-4 grid gap-3 sm:grid-cols-2">
          {shensha.map((rule,n) => (
            <article key={rule} className="rounded-xl border border-line bg-cream p-4">
              <span className="font-display text-xl text-earth">{String(n+1).padStart(2,"0")}</span>
      ...
      <section className="grid gap-4 sm:grid-cols-2">
        <article className="seal-border rounded-2xl bg-paper p-5 sm:p-7">
          <h3 className="font-display text-xl text-ink">{tr(locale,"古訓與網路整理要分開","古训与网络整理要分开","Separate classical quotations from modern summaries")}</h3>
          <div className="mt-4 flex flex-wrap gap-2">
            <span className="rounded-full border border-earth/30 bg-cream px-3 py-1 text-xs text-earth">{tr(locale,"古籍原文","古籍原文","Verified classical text")}</span>
            <span className="rounded-full border border-line bg-cream px-3 py-1 text-xs text-ink-soft">{tr(locale,"今人整理／出處待考","今人整理／出处待考","Modern summary / source unverified")}</span>
          </div>
        ...
        <article className="seal-border rounded-2xl bg-paper p-5 sm:p-7">
          <h3 className="font-display text-xl text-ink">{tr(locale,"趣味測驗另行","趣味测验另行","Fun quizzes stay separate")}</h3>
