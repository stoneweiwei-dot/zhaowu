...
export function OwnerGalleryManager({ session, locale }: { session: SupabaseSession; locale: Locale }) {
  ...
  const copy = useMemo(() => ({
    ...
    compactLead: tr(locale, "圖片預設收合，不再整頁鋪開。需要管理時再展開。", "图片默认收合，不再整页铺开。需要管理时再展开。", "Images stay collapsed by default instead of filling the whole page. Open the drawer only when you need to manage them."),
  ...
  return (
    <section className="seal-border rounded-[1.6rem] bg-cream/95 p-5 sm:p-7">
      ...
      <details
        data-owner-gallery-drawer
        open={open}
        onToggle={(event) => setOpen(event.currentTarget.open)}
        className="mt-5 overflow-hidden rounded-2xl border border-line bg-paper/32"
      >
        <summary className="flex min-h-16 cursor-pointer list-none items-center justify-between gap-4 px-4 py-3 marker:hidden sm:px-5">
          ...
          <span className="flex items-center gap-2 text-xs text-ink-mute">
            <span>{copy.atlas} {atlasAssets.length}</span>
            <span>·</span>
            <span>{copy.all} {assets.length}</span>
        ...
        {open ? (
          <div className="border-t border-line px-4 pb-5 pt-4 sm:px-5">
            <div className="flex gap-2" aria-label={tr(locale, "圖庫檢視", "图库检视", "Gallery view")}>
              {(["atlas", "all"] as OwnerView[]).map((item) => (
                <button
                  ...
                  className={`min-h-10 rounded-full border px-4 text-sm ${view === item ? "border-cinnabar/50 bg-cinnabar text-cream" : "border-line bg-cream text-ink-soft"}`}
                >
                  ...
                  <div className="p-3">
                    ...
                    <div className="mt-3 flex flex-wrap items-center justify-between gap-2">
                      ...
                      <button
                        ...
                        className="rounded-full px-2 py-1 text-[11px] text-cinnabar"
                      >
                        {copy.remove}
                      </button>
                    </div>
