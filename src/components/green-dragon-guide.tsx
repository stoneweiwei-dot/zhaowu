...
import { askSiteGuide, defaultSiteGuide, resolveLocalSiteGuide, type SiteGuideAnswer, type SiteGuideRoute } from "@/lib/site-guide";
...
export function GreenDragonGuide() {
  ...
  const copy = locale === "en"
    ? { title: "Jade Dragon guide", intro: "Tell me what you want to do. I will take you to the right page.", placeholder: "Where can I find my saved report?", ask: "Ask", close: "Close guide", open: "Open Jade Dragon guide", limit: "AI questions are limited to three per browser each day. The quick links still work." }
    : locale === "zh-Hans"
      ? { title: "青玉小龙导航", intro: "告诉我你想做什么，我带你去正确页面。", placeholder: "例如：我想看保存的报告", ask: "问小龙", close: "关闭导航", open: "打开青玉小龙导航", limit: "每个浏览器每天可问 AI 三次；下方快捷入口仍可使用。" }
      : { title: "青玉小龍導覽", intro: "告訴我你想做什麼，我帶你去正確頁面。", placeholder: "例如：我想看保存的報告", ask: "問小龍", close: "關閉導覽", open: "打開青玉小龍導覽", limit: "每個瀏覽器每天可問 AI 三次；下方快捷入口仍可使用。" };
    ...
    <aside className="zhaowu-dragon-guide" data-site-guide>
      {open ? (
        <section className="zhaowu-dragon-guide-panel" role="dialog" aria-label={copy.title}>
          ...
          <div className="zhaowu-dragon-guide-shortcuts" aria-label="Site shortcuts">
            <button type="button" onClick={() => go("/#analysisForm")}>{locale === "en" ? "Analysis" : locale === "zh-Hans" ? "开始分析" : "開始分析"}</button>
            <button type="button" onClick={() => go("/tianji-dual")}>{locale === "en" ? "Dual chart" : locale === "zh-Hans" ? "双轨命盘" : "雙軌命盤"}</button>
            <button type="button" onClick={() => go("/account")}>{locale === "en" ? "My reports" : locale === "zh-Hans" ? "我的报告" : "我的報告"}</button>
          </div>
      ...
      <button className="zhaowu-dragon-guide-trigger" type="button" onClick={() => setOpen((value) => !value)} aria-expanded={open} aria-label={open ? copy.close : copy.open}>
        <span className="zhaowu-dragon-guide-avatar" aria-hidden />
        <span>{locale === "en" ? "Guide" : locale === "zh-Hans" ? "问小龙" : "問小龍"}</span>
      </button>
    </aside>
  );
}
