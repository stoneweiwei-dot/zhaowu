...
function Home() {
  ...
  const dualCopy =
    locale === "en"
      ? {
          title: "Two sides of character",
          detail: "How you act day to day and respond under pressure",
          mark: "II",
        }
      : locale === "zh-Hans"
        ? {
            title: "性格两面",
            detail: "平时怎样做事，压力来时怎样反应",
            mark: "两面",
          }
        : {
            title: "性格兩面",
            detail: "平時怎樣做事，壓力來時怎樣反應",
            mark: "兩面",
          };
  ...
  return (
    <main className="zhaowu-home-sheet-page space-y-7 sm:space-y-12">
      ...
      <Link
        to="/tianji-dual"
        className="zhaowu-home-dual-entry"
        aria-label={dualCopy.title}
      >
        <span className="zhaowu-home-dual-mark" aria-hidden="true">
          {dualCopy.mark}
        </span>
        <span className="zhaowu-home-dual-copy">
          <strong>{dualCopy.title}</strong>
          <small>{dualCopy.detail}</small>
        </span>
