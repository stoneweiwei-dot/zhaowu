  ...
  const funCopy =
    locale === "en"
      ? {
          ...
          cards: [
            { series: "SERIES 01", title: "Inner Animal × Guardian Beast", know: "The personality strategy you use most now and your instinctive response under pressure.", bestAt: "Learned behaviour and decision style; if a BaZi chart exists, it can also be compared with your guardian-beast pattern." },
            { series: "SERIES 02 · FIVE-ELEMENT FUNCTION TEST", title: "Which function do you need to strengthen right now?", know: "Whether you currently need more growth, activation, follow-through, boundaries or recovery.", bestAt: "Turning a recent bottleneck into one practical function you can train now." },
          ],
        }
      : locale === "zh-Hans"
        ? {
            ...
            action: "开始测验",
            cards: [
              { series: "SERIES 01", title: "内在动物 × 命局瑞兽", know: "你现在最常使用的人格策略，以及压力下最自然的反应。", bestAt: "后天行为偏好与决策方式；已有命盘时，可再和命局瑞兽交叉看。" },
              { series: "SERIES 02 · 五行功能测验", title: "你现在最需要练哪一行？", know: "你现在更需要生长、启动、落地、收敛，还是恢复。", bestAt: "把最近 1–3 个月的卡点，转成一个可以立即练习的功能方向。" },
            ],
          }
        : {
            kicker: "趣味測驗",
            ...
            action: "開始測驗",
            cards: [
              { series: "SERIES 01", title: "內在動物 × 命局瑞獸", know: "你現在最常使用的人格策略，以及壓力下最自然的反應。", bestAt: "後天行為偏好與決策方式；已有命盤時，可再和命局瑞獸交叉看。" },
              { series: "SERIES 02 · 五行功能測驗", title: "你現在最需要練哪一行？", know: "你現在更需要生長、啟動、落地、收斂，還是恢復。", bestAt: "把最近 1–3 個月的卡點，轉成一個可以立即練習的功能方向。" },
            ],
          };
      ...
      <section className="seal-border rounded-2xl bg-cream/90 p-5 sm:p-7" aria-label={funCopy.title}>
        ...
        <div className="mt-5 grid gap-4 sm:grid-cols-2">
          {funCopy.cards.map((card) => (
            <Link key={card.series} to="/fun-tests" className="block rounded-2xl border border-line bg-paper p-5 transition hover:border-cinnabar/50" aria-label={card.title}>
