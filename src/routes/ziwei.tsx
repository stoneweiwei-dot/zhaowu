...
function ZiweiPage() {
  ...
  const copy = locale === "en"
    ? {
        ...
        trust: "Calculation layer v0.5 · star positions are not generated by AI", input: "Birth details", year: "Year", month: "Month", day: "Day", hour: "Hour", minute: "Minute",
        ...
        source: "Engine status", sourceBody: "The calculation is deterministic and pinned to a reference implementation. Primary-source Golden Test verification is still in progress, so the truth layer remains marked productionReady=false internally.",
    ...
    : locale === "zh-Hans"
      ? {
          ...
          trust: "计算层 v0.5 · 星位不由 AI 生成", input: "出生资料", year: "年", month: "月", day: "日", hour: "时", minute: "分",
          ...
          source: "引擎状态", sourceBody: "排盘已经使用固定函数并锁定参考实现；原典 Golden Test 仍在继续核验，所以底层内部继续保持 productionReady=false，不冒充完全认证。",
      ...
      : {
          ...
          trust: "計算層 v0.5 · 星位不由 AI 生成", input: "出生資料", year: "年", month: "月", day: "日", hour: "時", minute: "分",
          ...
          source: "引擎狀態", sourceBody: "排盤已經使用固定函數並鎖定參考實作；原典 Golden Test 仍在繼續核驗，所以底層內部繼續保持 productionReady=false，不冒充完全認證。",
    ...
    <main className="ziwei-page">
      ...
      {result ? (
        <section id="ziwei-result" className="ziwei-result">
          ...
          <section className="ziwei-engine-note"><p>{copy.source}</p><strong>ziwei_truth_extension_v0.5</strong><span>{copy.sourceBody}</span><code>iztro 2.6.0 · 1ba89cca…</code></section>
        </section>
      ):null}
    </main>
  );
